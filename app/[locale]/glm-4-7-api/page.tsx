...
const pagePath = "/glm-4-7-api";

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "metadata.glm47Api" });
...
export default async function GLM47APIPage() {
  const t = await getTranslations("landing");
  const faqItems = t.raw("glm47Api.faq.items") as Array<{ q: string; a: string }>;

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Article",
    "headline": t("glm47Api.hero.title"),
    "description": t("glm47Api.hero.description"),
    "author": {
      "@type": "Organization",
      "name": "GLM API Proxy"
    },
        ...
        <nav className="bg-gray-50 border-b border-gray-200 px-6 py-3">
          <div className="mx-auto max-w-4xl">
            <ol className="flex items-center space-x-2 text-sm text-gray-600">
              ...
              <li>/</li>
              <li className="text-gray-900 font-medium">{t("glm47Api.breadcrumb")}</li>
            </ol>
          </div>
        </nav>
        ...
        <section className="px-6 py-16 bg-gradient-to-br from-emerald-50 via-white to-emerald-50">
          <div className="mx-auto max-w-4xl">
            <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-6">
              {t("glm47Api.hero.title")}
            </h1>
            <p className="text-xl text-gray-600 leading-relaxed">
              {t("glm47Api.hero.description")}
            </p>
          </div>
        </section>
        ...
        <section className="px-6 py-16 bg-gradient-to-br from-emerald-600 to-emerald-800">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-3xl font-bold text-white mb-4">
              {t("glm47Api.cta.title")}
            </h2>
            <p className="text-lg text-emerald-100 mb-8">
              {t("glm47Api.cta.description")}
            </p>
